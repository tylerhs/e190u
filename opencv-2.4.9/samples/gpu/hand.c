...
void filter_and_threshold(struct ctx *ctx)
{
	...
	cvInRangeS(ctx->temp_image3,
		   cvScalar(0, 55, 40, 255),
		   cvScalar(40, 175, 120, 255),
		   ctx->thr_image);
...
void find_contour(struct ctx *ctx)
{
    ...
    cvRectangle(ctx->temp_image1, cvPoint(0,0), cvPoint(cvGetSize(ctx->image).width/2,cvGetSize(ctx->image).height), BLACK,  CV_FILLED,8, 0 );
    ...
    /* cvFindContours modifies input image, so make a copy */
	cvCopy(ctx->thr_image, ctx->temp_image1, NULL);
    cvRectangle(ctx->temp_image1, cvPoint(cvGetSize(ctx->image).width/2,0), cvPoint(cvGetSize(ctx->image).width,cvGetSize(ctx->image).height), BLACK,  CV_FILLED,8, 0 );

	cvFindContours(ctx->temp_image1, ctx->temp_st, &contours2,
		       sizeof(CvContour), CV_RETR_EXTERNAL,
		       CV_CHAIN_APPROX_SIMPLE, cvPoint(0, 0));
...
void find_convex_hull(struct ctx *ctx)
{
    ...
    if (ctx->hull2) {
		...
		if (defects2 && defects2->total) {
			defect_array2 = calloc(defects2->total,
					      sizeof(CvConvexityDefect));
...
void display(struct ctx *ctx)
{
    ...
    if (1 == 1){
		...
		cvDrawContours(ctx->image, ctx->contour, BLUE, GREEN, 0, 1,
			       CV_AA, cvPoint(0, 0));
        cvDrawContours(ctx->image, ctx->contour2, BLUE, GREEN, 0, 1,
			       CV_AA, cvPoint(0, 0));
#endif
		cvCircle(ctx->image, ctx->hand_center, 5, RED, 1, CV_AA, 0);
		cvCircle(ctx->image, ctx->hand_center, ctx->hand_radius,
			 RED, 1, CV_AA, 0);
        cvCircle(ctx->image, ctx->hand_center2, 5, GREEN, 1, CV_AA, 0);
		cvCircle(ctx->image, ctx->hand_center2, ctx->hand_radius2,
			 GREEN, 1, CV_AA, 0);
